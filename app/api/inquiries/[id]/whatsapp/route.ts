...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!buttonType || !["welcome", "followup", "reminder"].includes(buttonType)) {
    ...
    // Check if this message type has already been sent for this inquiry
    const existing = await sql`
      SELECT id FROM whatsapp_status 
      WHERE record_id = ${inquiryId} 
      AND record_type = 'inquiry' 
    ...
    await sql`
      ...
      VALUES (${inquiryId}, 'inquiry', ${name}, ${phone}, ${buttonType}, ${message})
    `
